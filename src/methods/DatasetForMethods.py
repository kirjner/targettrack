...
class Dataset:
    ...
    def rename_data(self, name_before, name_after, overwrite=False):
        ...
        if self.suffix == "h5":
            ...
            dsnew = self.data.create_dataset(
                name_after, ds.shape, ds.dtype, ds.compression
            )
            dsnew = ds
            del ds
        ...
        if self.suffix == "h5":
            ...
            if key not in self.data.keys():
                ds = self.data.create_dataset(
                    key, frame.shape, frame.dtype, compression=compression
                )
                ds[...] = frame
            else:
                if not shape_change:
    ...
    def set_points(self, points=None):
        ...
        if self.suffix == "h5":
            if points is None:
                points = np.full(
                    (self.data.attrs["T"], self.data.attrs["N_points"] + 1, 3),
                    np.nan,
                    dtype=np.float32,
                )
            if "pointdat" not in self.data.keys():
                ds = self.data.create_dataset(
                    "pointdat", shape=points.shape, dtype=points.dtype
                )
                ds[...] = points
            else:
    ...
    def set_helper(self, name, helper):
        ...
        if self.suffix == "h5":
            ...
            if key not in self.data.keys():
                ds = self.data.create_dataset(
                    key, shape=helper.shape, dtype=helper.dtype
                )
                ds[...] = helper
            else:
        ...
        if self.suffix == "h5":
            if "pointdat" not in self.data.keys():
                ...
                points = np.full(
                    (
                        self.data.attrs["T"],
                        self.data.attrs["N_points"] + n_add + 1,
                        3,
                    ),
                    np.nan,
                    dtype=np.float32,
                ...
                points = np.concatenate(
                    [points, np.full((points.shape[0], n_add, 3), np.nan)],
                    axis=1,
                )
