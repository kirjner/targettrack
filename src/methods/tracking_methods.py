...
class NNTA:
    ...
    def run(self, file_path):
        ...
        if True:
            ...
            grid = np.stack(
                np.meshgrid(np.arange(W), np.arange(H), np.arange(D), indexing="ij"),
                axis=0,
            )
            ...
            for t in range(T):
                ...
                if valid.sum() >= min_points:
                    ...
                    maskpts = NNtools_points.get_maskpts(
                        inds_to_labels[inds],
                        pts[inds],
                        gridpts,
                    ...
                    torch.save(
                        mask,
                        os.path.join(self.folpath, "masks", str(t) + ".pt"),
                    )
        ...
        if self.params["Targeted"] and (
            (not self.dataset.exists("distmat")) or self.params["recalcdistmat"]
        ):
            self.state = ["Embedding Posture Space Training", 0]
            self.encnet = Networks.AutoEnc2d(
                sh2d=(W, H), n_channels=n_channels, n_z=min(20, T // 4)
            )
            ...
            loader = torch.utils.data.DataLoader(
                data,
                batch_size=self.params["batch_size_posture"],
                shuffle=True,
                num_workers=0,
                pin_memory=True,
            )
            ...
            while not traindone:
                for i, ims in enumerate(loader):
                    ...
                    if self.params["verbose"]:
                        print(
                            "\r" + self.state[0] + " " + str(self.state[1]),
                            end="",
                        )
            ...
            with torch.no_grad():
                for i in range(T):
                    ...
                    if self.cancel:
                        self.quit()
                        return
                    _, latent = self.encnet(
                        data[i].unsqueeze(0).to(device=self.device, dtype=torch.float32)
                    )
        ...
        if True:
            ...
            self.net = getattr(Networks, self.params["network"])(
                n_channels=n_channels, num_classes=N_labels
            )
            ...
            for epoch in range(n_epochs):
                for i, onebatch in enumerate(loader):
                    if self.cancel:
                        self.quit()
                        return
                    loss = NNtools_points.single_batch(
                        onebatch,
                        self.net,
                        NNtools_points.selective_ce,
                        opt,
                        self.device,
                    )
        ...
        if self.params["Targeted"]:
            ...
            if True:
                ...
                evaldata = NNtools_points.EvalDataset(
                    folpath=self.folpath, T=T, mask=True, maxz=False
                )
                ...
                while True:
                    if self.params["num_additional"] == 0:
                        break
                    self.state[1] = int(
                        100 * ((n_added) / self.params["num_additional"])
                    )
                    if self.params["verbose"]:
                        print(
                            "\r" + self.state[0] + " " + str(self.state[1]),
                    ...
                    # getting points
                    with torch.no_grad():
                        im = evaldata[i_tar][0].to(
                            device=self.device, dtype=torch.float32
                        )
                        maskpred = (
                            torch.argmax(self.net(im.unsqueeze(0))[0], dim=0)
                            .cpu()
                            .detach()
                            .numpy()
                        )
                        ...
                        if self.params["fast_points"]:
                            ...
                            nonback = maskpred > 0
                            if np.sum(nonback) != 0:
                                pts = NNtools_points.get_point_fast3D(
                                    maskpred[nonback],
                                    grid_dimend[nonback],
                                    N_labels,
                                )
                            ...
                            if self.params["weight_channel"] is None:
                                pts_dict = NNtools_points.get_pts_dict(
                                    maskpred,
                                    grid,
                                ...
                                pts_dict = NNtools_points.get_pts_dict(
                                    maskpred,
                                    grid,
                                    weight=im[self.params["weight_channel"]]
                                    .cpu()
                                    .detach()
                                    .numpy(),
                                )
                    ...
                    deformation, success = Deformation.get_deformation(
                        name=self.params["deformation_name"],
                        ptfrom=pts_gt,
                        ptto=pts_tar,
                        sh=(W, H, D),
                        device=self.device,
                        **self.params["deformparams"]
                    )
                    if not success:
                        continue
                    with torch.no_grad():
        ...
        if True:
            ...
            name = (
                "NN"
                + ("_" if self.params["save_suff"] != "" else "")
                + self.params["save_suff"]
            )
