...
def getpad(ten, minshape):
    ...
    if len(sh) == 4:
        W, H = sh[2], sh[3]
        return (
            minshape[0] * (W // minshape[0] + (W % minshape[0] != 0)) - W,
            minshape[1] * (H // minshape[1] + (H % minshape[1] != 0)) - H,
        )
    elif len(sh) == 5:
        ...
        return (
            minshape[0] * (W // minshape[0] + (W % minshape[0] != 0)) - W,
            minshape[1] * (H // minshape[1] + (H % minshape[1] != 0)) - H,
            minshape[2] * (D // minshape[2] + (D % minshape[2] != 0)) - D,
        )
    else:
...
class ASPP3D(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates):
        ...
        self.compressionconv = nn.Conv3d(
            (len(atrous_rates) + 1) * out_channels, out_channels, 1, bias=False
        )
...
class ASPP2D(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates):
        ...
        self.compressionconv = nn.Conv2d(
            (len(atrous_rates) + 1) * out_channels, out_channels, 1, bias=False
        )
...
class ThreeDCN(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv3d(
            n_filt_in, n_filt, kernel_size=5, stride=1, padding=2, bias=False
        )
        self.norm1 = nn.BatchNorm3d(n_filt)
        self.conv2 = nn.Conv3d(
            n_filt, n_filt, kernel_size=5, stride=1, padding=2, bias=False
        )
        ...
        self.conv3 = nn.Conv3d(
            n_filt_in, n_filt, kernel_size=3, stride=1, padding=1, bias=False
        )
        self.norm3 = nn.BatchNorm3d(n_filt)
        self.conv4 = nn.Conv3d(
            n_filt, n_filt, kernel_size=3, stride=1, padding=1, bias=False
        )
        ...
        self.compression2conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        ...
        n_filt += growth
        self.ASPP3D2 = ASPP3D(
            n_filt_in, n_filt, atrous_rates=((3, 3, 1), (6, 6, 2), (9, 9, 3))
        )
        self.compressionASPP3D2conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        self.compressionASPP3D2norm = nn.BatchNorm3d(
            compress_targ
        )  # compress to send up
        # we already have up2
        ...
        n_filt += growth
        self.ASPP3D3 = ASPP3D(
            n_filt_in, n_filt, atrous_rates=((3, 3, 1), (6, 6, 1), (9, 9, 2))
        )
        self.compressionASPP3D3conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        self.compressionASPP3D3norm = nn.BatchNorm3d(
            compress_targ
        )  # compress to send up
        ...
        self.ASPP3D4 = ASPP3D(
            n_filt_in,
            n_filt,
            atrous_rates=((2, 2, 1), (3, 3, 1), (4, 4, 2), (5, 5, 2)),
        )
        self.compressionASPP3D4conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        self.compressionASPP3D4norm = nn.BatchNorm3d(
            compress_targ
        )  # compress to send up
        self.up4 = nn.Upsample(scale_factor=(16, 16, 4))

        self.conv_out = nn.Conv3d(
            n_channels + n_filt_init + (n_filt_init + growth) + 4 * compress_targ,
            num_classes,
            kernel_size=1,
            stride=1,
            bias=True,
    ...
    def forward(self, x, verbose=False):
        ...
        x = nn.functional.pad(
            x, pad=(padD, 0, padH, 0, padW, 0), mode="constant", value=0.0
        )
        ...
        send2 = self.up2(
            self.relu(self.compression2norm(self.compression2conv(x)))
        )  # 64->save memory
        # compress_targ

        x = self.ASPP3D2(x)
        send2ASPP3D = self.up2(
            self.relu(self.compressionASPP3D2norm(self.compressionASPP3D2conv(x)))
        )
        x = self.down(x)

        # remember we don't down z 32x20x4
        x = self.ASPP3D3(x)
        send3ASPP3D = self.up3(
            self.relu(self.compressionASPP3D3norm(self.compressionASPP3D3conv(x)))
        )
        x = self.down_noz(x)

        # remember we don't down z 16x10x4
        x = self.ASPP3D4(x)
        send4ASPP3D = self.up4(
            self.relu(self.compressionASPP3D4norm(self.compressionASPP3D4conv(x)))
        )

        # 1+32+(32+32)+(32)+4*32
        x = torch.cat(
            [ori, send0, send1, send2, send2ASPP3D, send3ASPP3D, send4ASPP3D],
            dim=1,
        )  # n_channel+32+64+32+32+32
...
class TwoDCN(nn.Module):
    def __init__(
        ...
        self.compression2conv = nn.Conv2d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        ...
        self.compressionASPP2D2conv = nn.Conv2d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        self.compressionASPP2D2norm = nn.BatchNorm2d(
            compress_targ
        )  # compress to send up
        # we already have up2
        ...
        self.compressionASPP2D3conv = nn.Conv2d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        self.compressionASPP2D3norm = nn.BatchNorm2d(
            compress_targ
        )  # compress to send up
        ...
        n_filt += growth
        self.ASPP2D4 = ASPP2D(
            n_filt_in, n_filt, atrous_rates=((2, 2), (3, 3), (4, 4), (5, 5))
        )
        self.compressionASPP2D4conv = nn.Conv2d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        self.compressionASPP2D4norm = nn.BatchNorm2d(
            compress_targ
        )  # compress to send up
    ...
    def forward(self, x, verbose=False):
        ...
        x = self.ASPP2D2(x)
        send2ASPP3D = self.up2(
            self.relu(self.compressionASPP2D2norm(self.compressionASPP2D2conv(x)))
        )
        x = self.down(x)

        x = self.ASPP2D3(x)
        send3ASPP3D = self.up3(
            self.relu(self.compressionASPP2D3norm(self.compressionASPP2D3conv(x)))
        )
        x = self.down(x)

        x = self.ASPP2D4(x)
        send4ASPP3D = self.up4(
            self.relu(self.compressionASPP2D4norm(self.compressionASPP2D4conv(x)))
        )
...
class AutoEnc2d(nn.Module):
    def __init__(self, sh2d, n_channels=3, n_z=20, out=torch.sigmoid):
        ...
        dim_in = dim_out
        dim_out = 16
        self.convt11 = nn.ConvTranspose2d(
            dim_in, dim_out, kernel_size=2, stride=2, bias=False
        )
