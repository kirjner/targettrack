...
assert (
    len(sys.argv) == 4
), "1st argument: dataset_h5_path, 2nd argument: neural_network_name, 3rd argument: runname"
...
try:
    ...
    grid = np.moveaxis(
        np.array(
            np.meshgrid(
                np.arange(h5.attrs["W"]),
                np.arange(h5.attrs["H"]),
                np.arange(h5.attrs["D"]),
                indexing="ij",
            )
        ),
        0,
    ...
    result = pool.imap(
        get_pts_iou,
        (pack(i) for i in range(h5.attrs["T"])),
        chunksize=chunksize,
    )
