...
class FourierDeformationPk(nn.Module):
    def __init__(
        ...
        self.ks = torch.tensor(
            np.sum(
                (self.f.kgrid * self.dimscale[:, None, None, None]) ** 2,
                axis=0,
            )
        ).repeat(self.f.feed_ind.size(0), *[1 for _ in range(self.vecdim)])[
            self.f.feed_ind
        ]
        self.defscale = torch.tensor(np.array(defscale) / self.dimscale).to(
            dtype=torch.float32
        )
        ...
        self.grid = torch.stack(
            torch.meshgrid(
                *[torch.arange(self.realshape[i]) for i in range(self.vecdim)]
            )
        )
    ...
    def forward(self, fr, mask=None, individual_seeds=False, single_batch=False):
        ...
        self.disp = self.f(
            seeds.reshape(seeds.size(0), self.vecdim, self.f.dim)
            * self.Pk[None, None, :]
            * self.defscale[None, :, None]
        )
        moved = (2 * ((self.grid[None] - self.disp) / self.normten) - 1).to(
            dtype=torch.float32, device=seeds.device
        )
        if self.vecdim == 2:
        ...
        fr_aug = torch.nn.functional.grid_sample(
            fr.to(torch.float32),
            moved.repeat(fr.size(0), *[1 for _ in range(self.vecdim + 1)]),
            mode="bilinear",
            padding_mode="border",
            align_corners=True,
        )
        ...
        if mask is not None:
            mask_aug = torch.nn.functional.grid_sample(
                ...
                moved.repeat(mask.size(0), *[1 for _ in range(self.vecdim + 1)]),
                mode="nearest",
                padding_mode="zeros",
                align_corners=True,
            )[:, 0].to(torch.long)
