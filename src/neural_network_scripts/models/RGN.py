...
def knn(x, k):
    ...
    idx = pairwise_distance.topk(k=k, dim=2)[
        1
    ]  # (batch_size, num_points, k) this picks largest
    return idx
...
class Net(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(2 * self.num_in_feat, 64, kernel_size=1, bias=False),
            self.bn1,
        ...
        self.conv6 = nn.Sequential(
            nn.Conv1d(64 * 3, self.dim_embed, kernel_size=1, bias=False),
            self.bn6,
            nn.LeakyReLU(negative_slope=0.2),
        )
        self.conv7 = nn.Sequential(
            nn.Conv1d(self.dim_embed + 64 * 3, 512, kernel_size=1, bias=False),
            self.bn7,
            nn.LeakyReLU(negative_slope=0.2),
        )
    ...
    def forward(self, x):
        ...
        x = get_graph_feature(
            x, k=self.k, spatial_dim=self.spatial_dim
        ...
        x = self.conv6(
            x
        )  # (batch_size, 64*3, num_points) -> (batch_size, self.dim_embed, num_points)
        x = x.max(dim=-1, keepdim=True)[
            0
        ]  # (batch_size, self.dim_embed, num_points) -> (batch_size, self.dim_embed, 1)
        ...
        x = self.conv9(
            x
        )  # (batch_size, 256, num_points) -> (batch_size, num_classes, num_points)
