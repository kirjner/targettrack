...
class Net(nn.Module):
    def __init__(
        ...
        self.compression5norm = nn.BatchNorm3d(compress_targ)
    ...
    def forward(self, x, verbose=False):
        ...
        send2 = x
        send2 = self.compression2norm(self.compression2conv(send2))
        ...
        send3 = x
        send3 = self.compression3norm(self.compression3conv(send3))
        ...
        send4 = x
        send4 = self.compression4norm(self.compression4conv(send4))
        ...
        send5 = self.up5(
            self.relu(self.compression5norm(self.compression5conv(x)))
        )  # 64
