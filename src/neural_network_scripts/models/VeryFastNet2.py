...
class Net(nn.Module):
    def __init__(self, n_channels=3, growth=24, compress_targ=64, num_classes=10):
        ...
        self.compression2conv = nn.Conv3d(
            n_filt, self.compress_targ, kernel_size=1, stride=1, bias=False
        )
        ...
        self.conv_out = nn.Conv3d(
            n_channels + growth + (2 * growth) + 4 * (self.compress_targh),
            num_classes,
            kernel_size=1,
            stride=1,
            bias=True,
    ...
    def forward(self, x, verbose=False):
        ...
        send2 = self.compression2norm(
            send2[:, : self.compress_targh]
            * torch.sigmoid(send2[:, self.compress_targh :])
        )
        ...
        send3 = self.compression3norm(
            send3[:, : self.compress_targh]
            * torch.sigmoid(send3[:, self.compress_targh :])
        )
        ...
        send4 = self.compression4norm(
            send4[:, : self.compress_targh]
            * torch.sigmoid(send4[:, self.compress_targh :])
        )
        ...
        send5 = self.compression5norm(
            send5[:, : self.compress_targh]
            * torch.sigmoid(send5[:, self.compress_targh :])
        )
