...
class Net(nn.Module):
    def __init__(
        ...
        self.conv_out = nn.Conv3d(
            n_channels + growth + 2 * growth + 3 * compress_targ,
            num_classes,
            kernel_size=1,
            stride=1,
            bias=True,
    ...
    def forward(self, x, verbose=False):
        ...
        send3 = self.up3(
            self.relu(self.compression3norm(self.compression3conv(x)))
        )  # 32
        x = self.down(x)
        ...
        send4 = self.up4(
            self.relu(self.compression4norm(self.compression4conv(x)))
        )  # 32

        x = torch.cat(
            [ori, send0, send1, send2, send3, send4], dim=1
