...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates):
        ...
        for rate in atrous_rates:
            DilConvs.append(ASPPConv(in_channels, out_channels, rate))
...
class Net(nn.Module):
    def __init__(
        ...
        self.compressionaspp2conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        ...
        self.compressionaspp3conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
        ...
        self.compressionaspp4conv = nn.Conv3d(
            n_filt, compress_targ, kernel_size=1, stride=1, bias=False
        )
    ...
    def forward(self, x, verbose=False):
        ...
        x = self.aspp2(x)
        send2aspp = self.up2(
            self.relu(self.compressionaspp2norm(self.compressionaspp2conv(x)))
        )
        x = self.down(x)
        ...
        x = self.aspp3(x)
        send3aspp = self.up3(
            self.relu(self.compressionaspp3norm(self.compressionaspp3conv(x)))
        )
        x = self.down_noz(x)
        ...
        x = self.aspp4(x)
        send4aspp = self.up4(
            self.relu(self.compressionaspp4norm(self.compressionaspp4conv(x)))
        )

        # 1+32+(32+32)+(32)+4*32
        x = torch.cat(
            [ori, send0, send1, send2, send2aspp, send3aspp, send4aspp], dim=1
