...
class TrainDataset(Dataset):
    def __init__(self, dirname, shape, meansub=False, high=False, maskdirname="masks"):
        ...
        self.indlist = [
            int(name.strip().split("/")[-1].split("_")[-1].split(".")[0])
            for name in self.filelist
        ]  # Todo cleaner with path.??
        ...
        self.fourier_augment_strong = FourierAugment.FourierDeformationPk(
            self.shape[1:],
            k_cut_dimless=2.5,
            Pk=lambda k: 2 / (k**2 + 0.1),
            dimscale=(1, 1, 5.0),
            defscale=(0.5, 0.5, 0.25),
        )
        self.fourier_augment_weak = FourierAugment.FourierDeformationPk(
            self.shape[1:],
            k_cut_dimless=2.5,
            Pk=lambda k: 1 / (k**3 + 0.1),
            dimscale=(1, 1, 5.0),
            defscale=(0.25, 0.25, 0.125),
        )
        self.p_four = 0.5
        ...
        if torch.rand(1).item() < p:
            ...
            if self.grid2d is None:
                self.grid2d = np.array(
                    np.meshgrid(np.arange(sh2d[0]), np.arange(sh2d[1]), indexing="ij")
                )
            if top_or_bottom:
    ...
    def get_trf(self, fr, mask, mode):
        ...
        if mode == "full":
            res = self.trffull(
                image=fr.transpose(1, 2, 0, 3).reshape(fr.shape[1], fr.shape[2], -1),
                **feed
            )
            res = self.cut(res, fr.shape[1:3], p=1)
            fr_aug = (
                res["image"]
                .reshape(fr.shape[1], fr.shape[2], fr.shape[0], -1)
                .transpose(2, 0, 1, 3)
            )
            mask_aug = res["mask"]
            fr_aug, mask_aug = torch.tensor(fr_aug), torch.tensor(mask_aug)
            if torch.rand(1).item() < self.p_four:
                fr_aug, mask_aug = self.fourier_augment_strong(
                    fr_aug, mask=mask_aug, single_batch=True
                )
        elif mode == "aff_cut":
            res = self.trfaff(
                image=fr.transpose(1, 2, 0, 3).reshape(fr.shape[1], fr.shape[2], -1),
            ...
            fr_aug, mask_aug = torch.tensor(fr_aug), torch.tensor(mask_aug)
            if torch.rand(1).item() < self.p_four:
                fr_aug, mask_aug = self.fourier_augment_weak(
                    fr_aug, mask=mask_aug, single_batch=True
                )
        elif mode == "aff":
            res = self.trfaff(
    ...
    def __getitem__(self, i):
        ...
        ii = self.indlist[i]
        fr = (
            np.load(self.dirname + "/frames/frame_" + str(ii) + ".npy")[: self.shape[0]]
            / 255
        )
        mask = np.load(
            os.path.join(self.dirname, self.maskdirname, "mask_" + str(ii) + ".npy")
        )
        if self.high:
...
def get_pts_iou(package):
    ...
    if weight:
        ...
        pts_dict, weight_dict = gen_tracked_points(
            fr, predmask, thres=thres, grid=grid, weight=weight
        )
    else:
        pts_dict = gen_tracked_points(
            fr, predmask, thres=thres, grid=grid, weight=weight
        )
    ...
    for i in range(1, len(Cells)):  # exclude zero
        ...
        for j in range(1, len(Cells)):
            if j != i:
                ...
                while (
                    (border[0] in Coor[0])
                    and (border[1] in Coor[1])
                    and (border[2] in Coor[2])
                ):
...
def registration_JV(points, reference, ctrl_pts):
    ...
    level = 4
    scales = [0.6, 0.3, 0.2, 0.1]
    lambdas = [0.1, 0.01, 0.001, 0.001]
    ...
    after_tps, x0, loss = run_multi_level(
        points, reference, ctrl_pts, level, scales, lambdas, iters
    )
...
class RGNTrainDataset(Dataset):
    def __init__(
        ...
        self.indlist = [
            int(name.strip().split("/")[-1].split("_")[-1].split(".")[0])
            for name in glob.glob(self.dirname + "/masks/*")
        ]
        self.indlist = np.array(self.indlist)
        self.grid = grid = np.array(
            np.meshgrid(*[np.linspace(-1, 1, s) for s in self.shape[1:]], indexing="ij")
        ).reshape(3, -1)
        self.num_frames_tot = len(self.indlist)
    ...
    def __getitem__(self, i):
        ...
        ii = self.indlist[i]
        fr_flat = (
            np.load(self.dirname + "/frames/frame_" + str(ii) + ".npy")[
                : self.shape[0]
        ...
        return torch.tensor(
            np.concatenate([pts, fr_flat[:, pts_ind]], axis=0)
        ), torch.tensor(mask_flat[pts_ind])
...
class RGNEvalDataset(Dataset):
    ...
    def __getitem__(self, ii):
        ...
        pts_inds = [
            self.point_ind_sample_single(fr_flat[self.select_channel])
            for _ in range(self.eval_rep)
        ]
        pts = [self.grid[:, pts_inds[i]] for i in range(self.eval_rep)]
        try:
            mask_flat = np.load(
                self.dirname + "/masks/mask_" + str(ii) + ".npy"
            ).flatten()
            labs = torch.tensor(
                np.array([mask_flat[pts_inds[i]] for i in range(self.eval_rep)])
            )
        except FileNotFoundError:
            labs = None
        return (
            torch.tensor(
                np.concatenate(
                    [
                        np.array(pts),
                        np.array(
                            [fr_flat[:, pts_inds[i]] for i in range(self.eval_rep)]
                        ),
                    ],
                    axis=1,
                )
...
def get_deformation(
    ...
    locs_gridded = (
        2
        * (
            torch.tensor(locs)[:, None, None, None, :]
            / (np.array([W, H, D])[None, None, None, None, :] - 1)
        )
        - 1
    )
    ...
    for iters in range(iterations):
        deformation = f(x)
        deformation = torch.mean(deformation, dim=4, keepdim=True).repeat(
            1, 1, 1, 1, deformation.size(4)
        )
        vecs_sampled = torch.nn.functional.grid_sample(
            deformation.repeat(locs_gridded.size(0), 1, 1, 1, 1),
            locs_gridded,
            mode="bilinear",
            padding_mode="border",
            align_corners=True,
...
def deform(sh, deformation, fr, mask=None):
    W, H, D = sh
    grid = torch.stack(torch.meshgrid(*[torch.arange(s) for s in (W, H, D)])).to(
        dtype=torch.float32, device=deformation.device
    )
    normten = (torch.tensor([W, H, D])[None, :, None, None, None] - 1).to(
        dtype=torch.float32, device=deformation.device
    )
    moved = (2 * ((grid[None] - deformation) / normten) - 1).to(
        dtype=torch.float32, device=deformation.device
    )
    moved = moved.permute(0, 2, 3, 4, 1)[..., [2, 1, 0]]
    fr_aug = torch.nn.functional.grid_sample(
        fr.to(torch.float32),
        moved.repeat(fr.size(0), 1, 1, 1, 1),
        mode="bilinear",
        padding_mode="border",
        align_corners=True,
    )
    if mask is not None:
        mask_aug = torch.nn.functional.grid_sample(
            mask.unsqueeze(1).to(torch.float32),
            moved.repeat(mask.size(0), 1, 1, 1, 1),
            mode="nearest",
            padding_mode="zeros",
            align_corners=True,
        )[:, 0].to(torch.long)
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        if verbose:
            print(
                "Epoch: " + str(epoch) + " lr: " + str(optimizer.param_groups[0]["lr"])
            )
        net.train()
        eploss = 0
        count = 0
        for i, (fr, mask) in enumerate(traindataloader):
            ...
            txt = (
                ...
                + str(loss.item())
                + " nanmeaniou: "
                + str(np.nanmean(ious) if np.sum(np.isnan(ious) != 1) > 0 else np.nan)
            )
            if verbose:
                print(txt)
            log += txt + "\n"
        ...
        if vnum > 0:
            ...
            for i, (fr, mask) in enumerate(valdataloader):
                ...
                txt = (
                    ...
                    + " nanmeaniou: "
                    + str(
                        np.nanmean(ious) if np.sum(np.isnan(ious) != 1) > 0 else np.nan
                    )
                )
                if verbose:
                    print(txt)
        ...
        if write_log:
            logform = (
                "Prepare={:04.4f} Train={:04.4f} Predict={:04.4f} GetPoints={:04.4f}"
            )
            with open(logfn, "w") as f:
                f.write(logform.format(1.0, epoch / num_epochs, 0.0, 0.0) + "\n")
