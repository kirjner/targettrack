...
dataset_name = os.path.split(dataset_path)[1].split(".")[
    0
]  # MB first split pathname into a pair.component [1] has no slash
...
deformInput = int(
    sys.argv[3]
)  # determines whether to generate the deformed frames or not
if deformInput == 1:
...
if deformation_trick:
    if not "distmat" in h5.keys():
        distmat = NNtools.Compute_distmat(
            h5, T, W, H, batch_size=20, n_z=31, n_channels=1
        )
...
write_log(logform.format(0.0, 0.0, 0.0, 0.0))
...
if (
    reusedirec is not None
):  # MB: use the dir of masks and frames for training if it already exists
    datadir = reusedirec
else:
...
try:
    ...
    if reusedirec is None or not os.path.exists(reusedirec):  # TODO: is this correct?
        ...
        for i in range(T):  # I think this unpacks all the segmented/mask frames -MB
            write_log(logform.format(min(i / T, 0.8), 0.0, 0.0, 0.0))
            fr = np.array(h5[str(i) + "/frame"]).astype(
                np.int16
            )  # TODO: access to dataset with methods?
            np.save(os.path.join(datadir, "frames", "frame_" + str(i) + ".npy"), fr)
            Ntot += 1

            if str(i) + "/high" in h5.keys():
                np.save(
                    os.path.join(datadir, "highs", "high_" + str(i) + ".npy"),
                    np.array(h5[str(i) + "/high"]).astype(np.int16),
                )
            else:
                np.save(
                    os.path.join(datadir, "highs", "high_" + str(i) + ".npy"),
                    np.full((1, W, H), 255).astype(np.int16),
                )

            if GetTrain == 0:
                if str(i) + "/mask" in h5.keys():
                    np.save(
                        os.path.join(datadir, "masks", "mask_" + str(i) + ".npy"),
                        np.array(h5[str(i) + "/mask"]).astype(np.int16),
                    )
                    Nans += 1
        if GetTrain == 1:
            ...
            NNname = list(
                h5["net"].keys()
            )  # the new network is NNname[0] so use NNname[1] to access previous networks training set
            traininds = h5["net"][NNname[k]].attrs[
                "traininds"
            ]  # train indices are saved as an attribute of the first dataset(run w/O deformation)
            for (
                j
            ) in (
                traininds
            ):  # placing the training frames and masks in the deformation folder
                np.save(
                    os.path.join(datadir, "masks", "mask_" + str(j) + ".npy"),
                    np.array(h5[str(j) + "/mask"]).astype(np.int16),
                )
                Nans += 1
            if DeforemeFrames == 1:
                ...
                if num_added_frames == 0:
                    for l in range(origfrNum, T):  # to add the deformed frames
                        np.save(
                            os.path.join(datadir, "masks", "mask_" + str(l) + ".npy"),
                            np.array(h5[str(l) + "/mask"]).astype(np.int16),
                        )
                        Nans += 1
                else:
                    for l in range(
                        origfrNum, origfrNum + num_added_frames
                    ):  # to add the deformed frames
                        np.save(
                            os.path.join(datadir, "masks", "mask_" + str(l) + ".npy"),
                            np.array(h5[str(l) + "/mask"]).astype(np.int16),
                        )
    ...
    if requeue:
        net.load_state_dict(
            NNtools.load_from_h5(h5[identifier + "/" + usenet])
        )  # MB: dictionary of h5[identifier] items where the values are converted to torch tensors.
    ...
    if True:  # GetTrain == 0 :
        ...
        if tnum == "all" or vnum == 0:
            ...
            if totnum == (tnum + vnum):
                ...
                tset, vset, _ = torch.utils.data.random_split(
                    allset, [tnum, vnum, totnum - tnum - vnum]
                )
        ...
        if GetTrain == 0:
            if vnum > 0:
                h5[identifier].attrs["Validinds"] = allset.indlist[
                    vset.indices
                ]  # MB added
            else:
    ...
    if requeue and not adiabatic_trick:
        assert all(
            np.array(h5[identifier].attrs["traininds"]) == traininds
        ), "traininds not matching"
    ...
    def get_ious(preds, mask, skip=False):
        ...
        for i in range(num_classes):
            ...
            if (
                torch.sum(thismask).item() == 0
            ):  # MB: the case when neuron i is absent in GT
                ioubins[i] = np.nan
                continue
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.3, patience=patience, min_lr=5e-5
    )
    ...
    if skiptrain:
        ...
        if not int(sys.argv[3]):
            for epoch in range(num_epochs):
                ...
                for i, (fr, mask) in enumerate(traindataloader):
                    ...
                    if verbose:
                        print(
                            ...
                            + str(loss.item())
                            + " nanmeaniou: "
                            + str("nan" if np.all(np.isnan(ious)) else np.nanmean(ious))
                        )
                    log += (
                        "    train"
                        + str(i + 1).zfill(digits)
                ...
                dset = h5[identifier].create_dataset(
                    "loss_iou",
                    (len(losses), 2 + len(ious)),
                    dtype="f4",
                    compression="gzip",
                )
                dset[...] = np.concatenate(
                ...
                if os.path.exists("STOP"):
                    break
                # log
                write_log(logform.format(1.0, (epoch + 1) / num_epochs, 0.0, 0.0))
            write_log(logform.format(1.0, 1.0, 0.0, 0.0))
        ...
        if deformation_trick:
            ...
            defTrick = int(
                sys.argv[7]
            )  # whether to use the def trick designed for the point data or the masks
            if verbose:
            ...
            if os.path.exists(dir_deformations):
                shutil.rmtree(
                    dir_deformations
                )  # remove the deformed frames from the previous runs
            ...
            for name in np.array(allset.filelist)[
                allset.real_ind_to_dset_ind(traininds)
            ]:  # placing the training frames and masks in the deformation folder
                i = int(os.path.split(name)[-1].split(".")[0].split("_")[1])
                shutil.copyfile(
                    os.path.join(datadir, "frames", "frame_" + str(i) + ".npy"),
                    os.path.join(
                        datadir,
                        "deformations",
                        "frames",
            ...
            if defTrick == 3:
                ...
                with torch.no_grad():
                    ...
                    plot_results = True
                    targeted_augmentation_objects3.targeted_augmentation(
                        h5,
                        num_additional,
                        datadir,
                        allset,
                ...
                additional_inds = NNtools.select_additional(
                    T, traininds, distmat, num_additional
                )[
                    len(traininds) :
                ]  # index of frames used for augmentation
                h5[identifier].attrs["Deforminds"] = additional_inds  # MB added
                evalset = NNtools.EvalDataset(
                    datadir, shape
                )  # gets the new frame(out side of training set) .npy files
                countpl = 0
                totFrameChecked = 0
                ExtframeCount = 0
                ...
                while num_additional > 0:
                    ...
                    for i in additional_inds:
                        with torch.no_grad():
                            ...
                            if ignoreNN == 1:
                                ...
                                predmask = (
                                    predmask.cpu().detach().numpy().astype(np.int16)
                                )  # prediction of the frame out side of training set

                            else:
                                pred = net(fr_i)  # prediction of the target frame
                                predmask = (
                                    torch.argmax(pred[0], dim=0)
                                    .cpu()
                                    .detach()
                                    .numpy()
                                    .astype(np.int16)
                        ...
                        if (
                            len(np.unique(predmask)) < (num_classes) - 2
                        ):  # choose masks that  arebetter
                            num_additional = num_additional + 1
                            print("frame not accepted")
                        else:
                            with torch.no_grad():
                                i_parent = traininds[
                                    np.argmin(distmat[traininds, i])
                                ]  # closest training set frame to frame i
                                fr, mask = evalset[i_parent]
                                maskTemp = mask.cpu().detach().numpy().astype(np.int16)
                                pts_parent, pts_child, ptaug = (
                                    NNtools.get_pts_from_masksJV(maskTemp, predmask)
                                )  # pts_child: the training set points that we are transforming.point_parent:training set points after tranformation. ptaug: goal of the transformation which is the predicted mask
                                ...
                                if countpl < 100 and ManualCheck == 1:
                                    ...
                                    axs[1].set_title(
                                        "deformed frame", fontsize=axisfontSize
                                    )
                                    axs[2].set_title(
                                        "target frame", fontsize=axisfontSize
                                    )
                                    fig2, axs2 = plt.subplots(4, 2)
                                    im00 = axs2[0, 0].imshow(
                                        np.max(fr_r, axis=2).T
                                    ...
                                    axs2[0, 0].scatter(
                                        ptaug[:, 0], ptaug[:, 1], c="w", s=1
                                    )
                                    axs[2].imshow(
                                        np.max(fr_r, axis=2).T
                                    )  # plot the target frame
                                    im01 = axs2[0, 1].imshow(
                                        np.max(predmask, axis=2).T
                                    )  # ,cmap="nipy_spectral",interpolation="none")
                                    fig.colorbar(im01, ax=axs2[0, 1])
                                    axs2[0, 1].set_ylabel(
                                        "target mask", rotation=0, labelpad=26
                                    )
                            transform_temp, loss_affine = NNtools.rotation_translation(
                                pts_child, pts_parent
                            )  # find a linear transformation+translation that takes the two sets of points to each other
                            ...
                            if countpl < 100 and ManualCheck == 1:
                                ...
                                axs2[3, 0].set_ylabel(
                                    "Rotated frame", rotation=0, labelpad=26
                                )

                            if defTrick == 1:
                                pts_Rot, pts_Rotts, ps_pred = (
                                    NNtools.get_pts_from_masksJV(maskTempRot, predmask)
                                )
                                deformation, _, _, success = NNtools.get_deformation(
                                    ptfrom=pts_Rot,
                                    ptto=pts_Rotts,
                                    sh=(W, H, D),
                                    k_cut_dimless=2,
                            ...
                            if defTrick == 2:
                                ...
                                if diag == 1:
                                    s = sim.generate_binary_structure(
                                        3, 3
                                    )  # considered together even if they touch diagonally
                                    labelArray, numFtr = sim.label(
                                        mask_warp2 > 0, structure=s
                                    )
                                else:
                                ...
                                for j in range(numFtr + 1):
                                    ...
                                    if len(list) == 2:
                                        ...
                                        mask_warp2[submask & submaskOrig1] = list[
                                            0
                                        ]  # np.max(np.unique(mask_warp2[submask]))
                                        mask_warp2[submask & submaskOrig2] = list[1]
                                        if (
                                            len(set(mask_warp2[submask]) - set(list))
                                            > 0
                                        ):
                                            coorX, coorY, coorZ = np.nonzero(
                                                np.array(submask)
                                            )
                                        ...
                                        mask_warp2[submask] = np.max(
                                            np.unique(mask_warp2[submask])
                                        )

                            if countpl < 100 and ManualCheck == 1:
                                im31 = axs2[3, 1].imshow(np.max(maskTempRot, axis=2).T)
                                fig.colorbar(im31, ax=axs2[3, 1])
                                fr_rParent = fr.unsqueeze(0).to(
                                    device=device, dtype=torch.float32
                                )  # added for plotting
                                ...
                                im11 = axs2[1, 1].imshow(
                                    np.max(mask.cpu().detach().numpy(), axis=2).T
                                )  # ,cmap="nipy_spectral",interpolation="none")
                            ...
                            if defTrick == 1:
                                ...
                                frB, mask = NNtools.deform(
                                    (W, H, D), deformation, fr3, mask=mask
                                )  # apply the deformation on the training frame and its mask
                                frC = np.clip(
                                    frB[0].cpu().detach().numpy() * 255, 0, 255
                                ).astype(np.int16)
                            ...
                            if defTrick == 2:
                                if countpl < 100 and ManualCheck == 1:
                                    ...
                                    im20 = axs2[2, 0].imshow(
                                        np.max(image1_warp, axis=2).T
                                    )
                                    axs2[2, 0].set_ylabel(
                                        "warped image", rotation=0, labelpad=26
                                    )
                                    fig.colorbar(im20, ax=axs2[2, 0])
                                    axs[1].imshow(np.max(image1_warp, axis=2).T)
                                    fig.subplots_adjust(wspace=0.270)

                                    im21 = axs2[2, 1].imshow(
                                        np.max(mask_warp2, axis=2).T
                                    )  # ,cmap="nipy_spectral",interpolation="none")
                                    axs2[2, 1].set_ylabel(
                                        "warped mask", rotation=0, labelpad=26
                                    )
                                ...
                                if checkCode == 1:
                                    ...
                                    if True:  # sfr not in traininds:
                                        fr, mask = evalset[sfr]
                                        mask = (
                                            mask.cpu().detach().numpy().astype(np.int16)
                                        )
                                        frC = np.clip(
                                            fr.cpu().detach().numpy() * 255,
                                            0,
                                            255,
                                        ).astype(
                                            np.int16
                            ...
                            if checkMB == 1:
                                ...
                                np.save(
                                    os.path.join(
                                        ...
                                        "deformations",
                                        "frames",
                                        "frame_" + str(T + ExtframeCount) + ".npy",
                                    ),
                                    frC,
                                )  # we add T to avoid collision, but don't want to define another class
                                np.save(
                                    os.path.join(
                                        datadir,
                                        "deformations",
                                        "masks",
                                        "mask_" + str(T + ExtframeCount) + ".npy",
                                    ),
                                    mask,
                                )
                                dset = h5.create_dataset(
                                    str(T + ExtframeCount) + "/frame",
                                    fr.shape,
                                    dtype="i2",
                                    compression="gzip",
                                )  # to save the mask in data set
                                dset[...] = frC
                                dset = h5.create_dataset(
                                    str(T + ExtframeCount) + "/mask",
                                    mask.shape,
                                    dtype="i2",
                                    compression="gzip",
                                )
                    ...
                    if num_additional > 0:  # if not enough frames were added
                        additional_inds = NNtools.select_additional(
                            ...
                            traininds,
                            distmat,
                            totFrameChecked + num_additional,
                        )[len(traininds) + totFrameChecked :]
                # now add the new masks
                ContinueNNWDef = 0  # int(input("Do you like to continue this NN training?(press 1 for yes) "))#13
                if ContinueNNWDef == 1:
                    allset = NNtools.TrainDataset(
                        os.path.join(datadir, "deformations"),
                        shape,
                        high=False,
                    )
                    traindataloader = torch.utils.data.DataLoader(
    ...
    if not int(sys.argv[3]) == 1:
        ...
        for i in range(T):
            ...
            with torch.no_grad():
                ...
                predmask = (
                    torch.argmax(pred[0], dim=0).cpu().detach().numpy().astype(np.int16)
                )

            if identifier + "/" + str(i) + "/predmask" in h5.keys():
                del h5[identifier + "/" + str(i) + "/predmask"]
            h5[identifier].create_dataset(
                str(i) + "/predmask",
                (predmask.shape),
                dtype="i2",
                compression="gzip",
            ...
            write_log(logform.format(1.0, 1.0, i / T, 0.0))

        # calculate prediction iou
        if "pred_iou" in h5[identifier].keys():
            del h5[identifier]["pred_iou"]
        dset = h5[identifier].create_dataset(
            "pred_iou", (T, num_classes), dtype="f4", compression="gzip"
        )
    ...
    write_log(logform.format(1.0, 1.0, 1.0, 1.0))
