...
if adiabatic_trick:
    ...
    def adiabatic_epoch_func(
        iters,
    ):  # function giving epochs for adiabatic trick
        min_adia_epochs = 4
...
gridpts = np.moveaxis(
    np.array(np.meshgrid(np.arange(W), np.arange(H), np.arange(D), indexing="ij")),
    0,
    3,
)
...
if adiabatic_trick:
    ...
    def get_pts(i):
        pts, _ = NNtools.get_pts_iou(
            NNtools.pack(h5, identifier, i, gridpts, num_classes, thres=thres)
        )
    ...
    def updatemask(i, pts):
        ...
        mask = NNtools.get_mask(
            fr[0], pts, num_classes, grid, thres=thres, distthres=distthres
        ).astype(np.int16)
...
try:
    ...
    if skiptrain:
        ...
        if deformation_trick:
            ...
            for i in additional_inds:
                with torch.no_grad():
                    ...
                    fr_r = np.clip(
                        fr[0, 0].cpu().detach().numpy() * 255, 0, 255
                    ).astype(
                        np.int16
                    )  # zero'th channel weighting
                    pts_child, _ = NNtools.get_pts_iou(
                        (
                            fr_r,
                            predmask,
                            None,
                ...
                fr = np.clip(fr[0].cpu().detach().numpy() * 255, 0, 255).astype(
                    np.int16
                )
            ...
            allset = NNtools.TrainDataset(
                os.path.join(datadir, "deformations"), shape, high=False
            )
            traindataloader = torch.utils.data.DataLoader(
                allset,
                batch_size=batch_size,
        ...
        if adiabatic_trick:  # more training steps for the trick
            ...
            assert all(
                (np.sort(np.array(traininds)) == np.sort(lineup[: len(traininds)]))
            ), "The first lineup elements should be traininds"
            repack()
            save_backup()  # make h5 backup
            ...
            for name in np.array(allset.filelist)[
                ...
                shutil.copyfile(
                    name,
                    os.path.join(datadir, "predmasks", os.path.split(name)[-1]),
                )

            evalset = NNtools.EvalDataset(
                datadir, shape
            )  # we need the evalutation set for aligned indices for all frames
            lineup = list(lineup)[
                len(traininds) :
            ]  # traininds should be at the begining
            ...
            while True:
                ...
                for ichild in ichilds:
                    ...
                    np.save(
                        os.path.join(
                            datadir,
                            "predmasks",
                            "mask_" + str(ichild) + ".npy",
                        ),
                        np.array(
                            h5[identifier + "/" + str(ichild) + "/predmask"]
                        ).astype(np.int16),
                    )
                ...
                # those in memory
                t_inds_memory = [
                    el for els in added[-short_memory:] for el in els
                ]  # last in memory
                t_inds_memory_dict = dict(
                    zip(
                        t_inds_memory,
                        [True for _ in range(len(t_inds_memory))],
                    )
                )
                # those not in memory
                buff = []
                ...
                for epoch in range(ad_epochs):
                    ...
                    t_inds_random_memory = list(
                        np.random.choice(
                            buff,
                            min(num_random_memory, len(buff)),
                            replace=False,
                        )
                    )
                    # random select random memory train
                    t_inds_random_memory_train = list(
                        np.random.choice(
                            traininds,
                            min(num_random_memory_train, len(traininds)),
                            replace=False,
                        )
                    )
                    # all tinds
                    tinds = [
                        *t_inds_memory,
                        *t_inds_random_memory,
                        *t_inds_random_memory_train,
                    ]
                    tset = torch.utils.data.Subset(
                        allset, allset.real_ind_to_dset_ind(tinds)
                    )
                    traindataloader = torch.utils.data.DataLoader(
                        tset,
                        batch_size=batch_size,
                    ...
                    if epoch in adiabatic_aug_dict.keys():
                        text = "augment is now:" + allset.change_augment(
                            adiabatic_aug_dict[epoch]
                        )
                        log += text
                        if verbose:
                            print(text)
    ...
    if get_points:
        ...
        result = pool.imap(
            NNtools.get_pts_iou,
            (
                NNtools.pack(h5, identifier, i, gridpts, num_classes, thres)
                for i in range(T)
            ),
            chunksize=chunksize,
        )
        for idx, res in enumerate(result):
            ...
            ious[idx] = iou
            write_log(logform.format(1.0, 1.0, 1.0, idx / T))
        ...
        write_log(logform.format(1.0, 1.0, 1.0, 0.1))
