...
class AnnotationData:
    ...
    def __init__(
        self, stem_savefile, frame_shape: tuple = (512, 512, 35)
    ):  # Todo: is it right to have a default value here?
        """
        ...
        try:
            ...
            self._normal_data_frame = pd.DataFrame(
                {"Time": [], "Segment": [], "x": [], "y": [], "z": []},
                dtype=int,
            )
        try:
            ...
            self._coarse_data_frame = pd.DataFrame(
                {"Time": [], "Segment": [], "x": [], "y": [], "z": []},
                dtype=int,
            )
        ...
        try:
            ...
            self.assignments = pd.DataFrame(
                {"Time": [], "Segment": [], "Clusternames": []}, dtype=int
            )
        self.new_format()
    ...
    def new_format(self):
        ...
        if not len(self.assignments):
            return
        if isinstance(
            self.assignments["Clusternames"][0], str
        ):  # no need to convert if they are already ints
            d = {
                name: i + 1
                for i, name in enumerate(self.assignments["Clusternames"].unique())
            }
    ...
    def segmented_frame(self, t, coarse=None):
        ...
        frame = sparse.COO(
            [segment_time.x, segment_time.y, segment_time.z],
    ...
    def get_mask(self, t=0, force_original=False):
        ...
        if force_original:
            ...
            df = self.data_frame
        if (
            t not in df.Time.values or t not in self.assignments.Time.values
        ):  # Todo: make sure t not in self.assignments.Time is not needed, as that should never happen
            raise KeyError
    ...
    def assign(self, assignment_dict, update_nb_neurons=False):
        ...
        self.assignments.drop_duplicates(
            keep="last", inplace=True, subset=["Time", "Segment"]
        )

    def to_file(self):
        self._normal_data_frame.to_csv(
            self._normal_seg_file, index=False, header=True, mode="w"
        )
        self._coarse_data_frame.to_csv(
            self._coarse_seg_file, index=False, header=True, mode="w"
        )
        self.assignments.to_csv(
            self.assignment_file, index=False, header=True, mode="w"
        )
