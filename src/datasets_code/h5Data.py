...
class h5Data(DataSet):
    def __init__(self, dataset_path=None):
        ...
        if "name" not in self.dataset.attrs:
            self.dataset.attrs["name"] = os.path.splitext(
        ...
        if "seg_params" not in self.dataset:
            ...
            for k, v in ParameterInitializer.new_parameters(
                "segmentation", "dummy"
            ).items():
        ...
        if "C" not in self.dataset.attrs:  # or self.dataset.attrs["C"] is None
            if "0/frame" in self.dataset:
                self.dataset.attrs["C"] = self.dataset["0/frame"].shape[
                    0
                ]  # number of channels
    ...
    def frame_shape(self):
        ...
        """
        return (
            self.dataset.attrs["W"],
            self.dataset.attrs["H"],
            self.dataset.attrs["D"],
        )
        ...
        if self.dataset.attrs["N_neurons"] == 0:
            ...
            for f in range(int(self.dataset.attrs["T"])):
                if "mask" in self.dataset[str(f)]:
                    neuronsNum = np.max(
                        [
                            len(np.unique(np.array(self.dataset[str(f) + "/mask"]))),
                            neuronsNum,
                        ]
                    )
                    count = count + 1
    ...
    def neuron_presence(self, value):
        ...
        if key not in self.dataset:
            self.dataset.create_dataset(
                key,
                (self.frame_num, self.nb_neurons + 1),
                dtype=bool,
                maxshape=(None, None),
            )
            ...
            try:
                ...
                RuntimeError
            ):  # cannot resize because maxshape was given at creation (can happen with older datasets)
    ...
    def real_neurites(self):
        return list(
            range(1, self.nb_neurons + 1)
        )  # Todo: nb_neurons must be up-to-date
    ...
    def copy_properties(self, other_ds, except_frame_num=False):
        assert isinstance(
            other_ds, h5Data
        ), "Cannot copy properties from other class of dataset"
    ...
    def segmented_times(self, force_regular_seg=False):
        ...
        if self.coarse_seg_mode and not force_regular_seg:
            ...
            mask_key = "mask"
        return [
            t for t in self.frames if str(t) + "/{}".format(mask_key) in self.dataset
        ]
    ...
    def segmented_frame(self, t, coarse=None):
        ...
        if not skey in self.dataset.keys():
            print(
                "segmentation not found for this frame "
            )  # MB added to extract the troubling frame
            print(t)
    ...
    def feature_array(
        ...
        if times is not None and ExtractFromSeg:
            ...
            segs = [
                (t, s) for t in times for s in np.unique(self.segmented_frame(t))[1:]
            ]
        elif times is not None and not ExtractFromSeg:
            print("get labels from mask")
            segs = [
                (t, s) for t in times for s in np.unique(self.get_mask(t))[1:]
            ]  # if you are using masks and masks are different from segs
        ...
        if rotation_invariant == 2:
            ...
            filt = range(
                11
            )  # todo: warning, the order may not be the same as the nd2 version
        print("id features: " + str(filt))

        print(segs)
        features = np.vstack(
            [
                np.array(self.dataset["features/{}/{}".format(t, s)])[filt]
                for t, s in segs
            ]
        )
        # Sanity check in case where the feature column becomes sparse (very few objects or variation present)
        feature_is_sparse = np.sum(np.isnan(features), axis=0) > (
            0.5 * features.shape[0]
        )
        if any(feature_is_sparse):
            warnings.warn(
                "{} sparse feature encountered and set to 0".format(
                    np.sum(feature_is_sparse)
                )
            )
    ...
    def get_segs_and_assignments(self, times):
        ...
        for t in times:
            ...
            for s in this_segs:
                id = np.argwhere(segmented[xs, ys, zs] == s)[
                    0
                ]  # first triplet that gives the coordinate of 1st pixel of segment s
                # neu = mask[xs, ys, zs][id[0], id[1], id[2]]#mask id of each present segment in frame t
                neu = mask[
                    xs[id], ys[id], zs[id]
                ]  # MB version,mask id of each present segment in frame t
                neus.append(neu)
        return segs, neus
    ...
    def original_intervals(self, which_dim=None):
        ...
        if group_key not in self.dataset or key not in self.dataset[group_key]:
            intervs = [
                [0, self.dataset.attrs["W"]],
                [0, self.dataset.attrs["H"]],
                [0, self.dataset.attrs["D"]],
            ]
    ...
    def replace_frame(self, t, img_red, img_green):
        ...
        self.dataset[str(t) + "/frame"][...] = np.stack([img_red, img_green]).astype(
            np.int16
        )
        orig_key = str(t) + "/oriframe"
        if orig_key not in self.dataset:
            self.dataset.create_dataset(
                orig_key, old_img.shape, dtype="i2", compression="gzip"
            )
    ...
    def _save_frame(self, t, frameR, frameG=0, mask=0):  # MB added
        ...
        if fkey not in self.dataset:
            self.dataset.create_dataset(
                fkey, frameTot.shape, dtype="i2", compression="gzip"
            )
        else:
        ...
        if np.any(mask):
            ...
            for key in (mkey, skey):
                if key in self.dataset:
                    del self.dataset[key]
                self.dataset.create_dataset(
                    key, mask.shape, dtype="i2", compression="gzip"
                )
    ...
    def flag_as_gt(self, frames):
        if "ground_truth" not in self.dataset.attrs:
            ...
            self.dataset.attrs["ground_truth"] = np.hstack(
                [self.dataset.attrs["ground_truth"], frames]
            )
    ...
    def save_features(self, t, s, ftr_dict):
        ...
        if key in self.dataset:
            del self.dataset[key]
        self.dataset.create_dataset(
            key, (len(list),), dtype="float", compression="gzip"
        )  # Todo: choose dtype?
    ...
    def _feature_dict_to_array(cls, ftr_dict):
        ...
        lst = [
            ftr_dict[lab]
            for lab in KEY  # ["Volume", "Red Total Intensity", "Red Intensity Var.", "Red Max. Intensity",
    ...
    def ca_act(self, ca_activity):
        ...
        """
        assert (self.nb_neurons == ca_activity.shape[0]) and (
            len(self.frames) == ca_activity.shape[1]
        ), "ci_int Shape mismatch"
        if "ci_int" in self.dataset:
            del self.dataset["ci_int"]
        self.dataset.create_dataset(
            "ci_int", shape=ca_activity.shape, dtype="f4", compression="gzip"
        )
