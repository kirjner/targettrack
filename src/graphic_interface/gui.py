...
class gui(QMainWindow):
    ...
    def __init__(self, controller, settings, parent=None):
        ...
        self.resize(
            self.settings["screen_w"] * 4 // 5,
            self.settings["screen_h"] * 4 // 5,
        )
        ...
        self.fig = plots.MainFigWidget(
            self.settings, self.controller, self.controller.frame_shape
        )
        main_layout.addWidget(self.fig)
        main_widget.setLayout(main_layout)
        self.setCentralWidget(main_widget)

        self.rendering = image_rendering.ImageRendering(
            self.controller,
            self.fig,
            self.controller.data_name,
            self.controller.frame_num,
        )
        ...
        neuron_bar = controls.NeuronBar(
            self.controller, reserved_keys=self.reserved_keys
        )
        ...
        slider = controls.TimeSlider(
            self.controller,
            self.controller.frame_num,
            int(self.settings["time_label_num"]),
        )
        ...
        if True:
            self.trackstable = controls.DashboardTab(
                self.controller,
                dashboard_chunk_size=int(self.settings["dashboard_chunk_size"]),
            )
            self.calacts = plots.ActivityPlotWidget(
                self.controller,
        ...
        if True:
            # Annotation tab
            annotation_tab = controls.AnnotationTab(
                self.controller,
                self.controller.frame_num,
            ...
            # NN control tab
            NN_control_tab = controls.NNControlTab(
                self.controller, self.controller.data_name
            )
            ...
            if not self.controller.point_data:
                ...
                # Export/Import tab
                export_import_tab = controls.ExportImportTab(
                    self.controller, self.controller.frame_num
                )
        ...
        self.rotating = False
        self.eventbucket["c"].activated.connect(
            lambda: self.activate_rotate(abort=False)
        )
        ...
        self.eventbucket["x"].activated.connect(
            lambda: self.activate_rotate(abort=True)
        )
