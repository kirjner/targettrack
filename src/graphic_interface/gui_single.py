...
class LazyDataset:
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            remote_data = self.conn.root.get_dataset_chunk(
                self.file_id, self.path, slice_info
            )
            ...
            logger.error(
                f"Error fetching dataset chunk for {self.path}, slices {slice_info}: {e}"
            )
            return None
        ...
        while attempt < self.max_retries:
            try:
                ...
                if "error" in server_info:
                    ...
                    device_type = (
                        "GPU" if server_info.get("cuda_available", False) else "CPU"
                    )
...
class RemoteH5File(DataSet):
    ...
    def _get_dataset(self, path: str) -> Optional[LazyDataset]:
        ...
        if path not in self._dataset_cache:
            dataset = LazyDataset(self.conn, self.file_id, path, self._chunk_cache)
            if dataset.exists:
    ...
    def pointdat(self):
        ...
        try:
            ...
            if dataset is not None:
                ...
                return np.full(
                    (self.frame_num, self.nb_neurons + 1, 3), np.nan
                )  # Default empty data
        except Exception as e:
    ...
    def set_point_data(self):
        ...
        try:
            ...
            self.conn.root.write_dataset_point_data(
                self.file_id, "/point_data", [self.point_data]
            )
    ...
    def send_pointdat_patch_to_server(self, frame, neuron, coord):
        ...
        try:
            ...
            self.conn.root.update_dataset_pointdat(
                self.file_id, path="/pointdat", patch_data=patch_data
            )
    ...
    def send_ci_int_patch_to_server(self, frame, settings):
        ...
        try:
            ...
            self.conn.root.exposed_update_dataset_ci_int_t(
                self.file_id, patch_data, settings
            )
    ...
    def ca_act(self, ca_activity: np.ndarray):
        ...
        try:
            ...
            assert (
                self.nb_neurons == ca_activity.shape[0]
            ), "Mismatch in number of neurons"
            assert (
...
class gui_single:
    def __init__(
        ...
        # Load settings
        self.settings = loaddict(
            os.path.join("src", "parameters", "current_settings.dat")
        )
        ...
        try:
            if tunneled:
                ...
                self.dataset = RemoteH5File(self.connection, dataset_path, node=node)
    ...
    def handle_server_disconnect(self):
        ...
        try:
            ...
            errdial = QErrorMessage()
            errdial.showMessage(
                "Lost connection to server. Please save your work and restart the application."
            )
            errdial.exec_()
