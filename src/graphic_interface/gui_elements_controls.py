...
class NeuronBar(QScrollArea):
    ...
    def __init__(self, controller, reserved_keys=None):
        ...
        self.removed_holder = (
            QWidget()
        )  # a holder, not displayed, to keep alive the widgets removed from other layouts
            ...
            for i in range(lay.count() - 1, -1, -1):
                ...
                if item.widget() is not None:
                    item.widget().setParent(
                        self.removed_holder
                    )  # or lay.removeItem(item) would be enough?
    ...
    def _make_user_neuron_key(self, neuron_id_from1):
        ...
        def fun():
            text, ok = QInputDialog.getText(
                self, "Select key for neuron " + str(neuron_id_from1), "Key: "
            )
...
class NeuronBarItem:
    ...
    def __init__(self, i_from1, parent):
        ...
        self.neuron_key_button.clicked.connect(
            self.parent._make_user_neuron_key(self.i_from1)
        )
        self.neuron_button = QPushButton(str(self.i_from1))
        self.neuron_button.setStyleSheet(self.qss)  # TODO set correct color NOW??
        self.neuron_button.clicked.connect(
            self.parent._make_neuron_highlight(self.i_from1)
        )
    ...
    def install_in(self, holder_layout):
        j = (
            holder_layout.count() // 2
        )  # this is the number of items already in the layout; there are two per neuron
    ...
    def set_present(self):
        ...
        self.neuron_button.setStyle(
            self.neuron_button.style()
        )  # for some reason this is needed to actually change the color
...
class ViewTab(QScrollArea):
    ...
    def __init__(self, controller, controlled_plot, default_values: Dict[str, bool]):
        ...
        first_channel_only_checkbox.setChecked(
            int(default_values["just_show_first_channel"])
        )  # MB changed bool to int
        first_channel_only_checkbox.toggled.connect(
            self.controller.toggle_first_channel_only
        )
        ...
        second_channel_only_checkbox.toggled.connect(
            self.controller.toggle_second_channel_only
        )
        ...
        gamma_slider.valueChanged.connect(
            lambda val: self.controlled_plot.change_gamma(val)
        )
        ...
        blend_slider_r.valueChanged.connect(
            lambda val: self.controlled_plot.change_blend_r(val)
        )
        ...
        blend_slider_g.valueChanged.connect(
            lambda val: self.controlled_plot.change_blend_g(val)
        )
        ...
        thres_slider_l.valueChanged.connect(
            lambda val: self.controlled_plot.change_low_thresh(val)
        )
        thres_slider_h = QSlider(Qt.Horizontal)
        thres_slider_h.setMinimum(0)
        thres_slider_h.setMaximum(100)
        thres_slider_h.setValue(100)
        thres_slider_h.valueChanged.connect(
            lambda val: self.controlled_plot.change_high_thresh(val)
        )
        ...
        if as_points:
            ...
            overlay_tracks_CheckBox.toggled.connect(
                self.controller.toggle_track_overlay
            )
            ...
            ov_tr_past.textChanged.connect(
                lambda x: self.controller.change_track_past(x)
            )
            ...
            ov_tr_future.textChanged.connect(
                lambda x: self.controller.change_track_future(x)
            )
            ...
            blur_slider_s.valueChanged.connect(
                lambda val: self.controlled_plot.change_blur_s(val)
            )
            ...
            blur_slider_b.valueChanged.connect(
                lambda val: self.controlled_plot.change_blur_b(val)
            )
...
class AnnotationTab(QScrollArea):
    ...
    def __init__(self, controller, frame_num: int, mask_threshold_for_new_region):
        ...
        if as_points:
            ...
            if True:
                ...
                getSize_AutoCenter.setStyleSheet(
                    "height: 15px; width: 5px;min-width: 5px;"
                )
                ...
                getSize_AutoCenterz.setStyleSheet(
                    "height: 15px; width: 5px;min-width: 5px;"
                )
                ...
                AutoCenterWidget2_btn.toggled.connect(
                    self.controller.toggle_autocenter_peakmode
                )

                autocenter_enable_lay = QHBoxLayout()
                autocenter_enable_lay.addWidget(QLabel("Autocenter Enabled [A]:"))
                self.auto_en_lab = QLabel("  ")
                self.auto_en_lab.setStyleSheet(
                    "background-color: green; height: 5px; width: 5px;min-width: 5px;"
                )
            ...
            mask_annotation_checkbox.toggled.connect(
                self.controller.toggle_mask_annotation_mode
            )
            ...
            self.mask_annotation_thresh.textChanged.connect(
                lambda x: self.controller.set_mask_annotation_threshold(x)
            )
            ...
            self.box_dimensions.setStyleSheet(
                "height: 15px; width: 5px;min-width: 5px;"
            )
            self.box_dimensions.textChanged.connect(
                lambda x: self.controller.set_box_dimensions(x)
            )
            ...
            self.cell_permutation_entry.setStyleSheet(
                "height: 15px; width: 5px;min-width: 5px;"
            )
            Permute_buttons.addWidget(
                QLabel("Enter cell numbers separated with ,"), 0, 0
            )
    ...
    def _selective_approve(self):
        fro, to = int(self.approve_select_from.text()), int(
            self.approve_select_to.text()
        )
    ...
    def change_highlighted_neuron(
        self, high: int = None, unhigh: int = None, high_key=None, **kwargs
    ):
        """
    ...
    def change_autocenter_mode(self, on: bool):
        ...
        if on:
            self.auto_en_lab.setStyleSheet(
                "background-color: green; height: 15px; width: 5px;min-width: 5px;"
            )
        else:
            self.auto_en_lab.setStyleSheet(
                "background-color: red; height: 15px; width: 5px;min-width: 5px;"
            )
...
class NNControlTab(QScrollArea):
    ...
    def __init__(self, controller, data_name: str):
        ...
        if as_points:
            ...
            self.importNN_address.setStyleSheet(
                "height: 15px; width: 5px;min-width: 5px;"
            )
            ...
            self.Exempt_Neurons.setStyleSheet(
                "height: 15px; width: 5px;min-width: 5px;"
            )
            main_layout.addWidget(
                QLabel("Neurons exempt from postprocessing:"), row, 0, 1, 1
            )  # TODO MB: exempt for modes 1-2, but target for modes 3-4-5, right? I think we can change the label. Possibly we can even change the label depending on which mode is selected.
            ...
            self.post_process_mode_name = [
                "merge neighbors",
                "merge neighbors XY",
                "merge selected cells",
                "delete disjoint",
                "relabel neighbors to 1st",
            ]
            for i in range(1, 6):
            ...
            val_frame_box = QtHelpers.CollapsibleBox(
                "Validation frames id:"
            )  # MB added
            lay = QVBoxLayout()
            ...
            self.NN_model_select.currentTextChanged.connect(
                self.populate_mask_NNinstances
            )
    ...
    def _run_mask_NN(self, fol=False):
        ...
        if instancename == "new":
            text, ok = QInputDialog.getText(
                self, "Name your instance", "Instance Name: "
            )  # TODO AD is self ok here or should it be another class of Qt thing?
            if not ok:
                return
        ...
        if confirmation == QMessageBox.Ok:
            runres, msg = self.controller.run_NN_masks(
                modelname,
                instancename,
                fol,
                int(self.epochs.text()),
                int(self.trainset.text()),
                int(self.valset.text()),
...
class SelectionTab(QScrollArea):
    ...
    def __init__(self, controller):
        ...
        selection_lay.addWidget(
            QLabel("Enter frame numb-\n ers separated\n with ,"), 1, 0
        )
    ...
    def _frame_fraction_fun(self):
        ...
        self.controller.select_frames(
            float(frac) / 100,
            Tot_fr_final,
            self.population_buttons.checkedButton().text(),
        )
...
class MaskProcessingTab(QScrollArea):
    ...
    def __init__(self, controller):
        ...
        params = [
            {"name": k, "value": v, **Parameters.pyqt_param_keywords(k)}
        ...
        params = [
            {"name": k, "value": v, **Parameters.pyqt_param_keywords(k)}
            for k, v in self.cluster_params.items()
        ]
        ...
        def change_cluster_pars(param, changes):  # Todo: as method
            for param, change, data in changes:
                self.cluster_params.update(
                    {param.name(): (eval(data) if isinstance(data, str) else data)}
                )
...
class ExportImportTab(QScrollArea):
    ...
    def __init__(self, controller, frame_num: int):
        ...
        reverse_transform_checkbox.toggled.connect(
            self.controller.toggle_reverse_transform
        )
        ...
        save_rotation_crop_checkbox.toggled.connect(
            self.controller.toggle_save_crop_rotate
        )
        ...
        bg_subtraction_checkbox.toggled.connect(
            self.controller.toggle_save_subtracted_bg
        )
    ...
    def _Preprocess_and_save(self):
        ...
        self.controller.Preprocess_and_save(
            frame_range,
            Tot_del_fr,
            Z_int,
            X_int,
...
class TimeSlider(LabeledSlider):
    def __init__(self, controller, T, n_labels):
        ...
        super(TimeSlider, self).__init__(
            minimum=0, maximum=T - 1, interval=int(T / n_labels)
        )
        self.sl.valueChanged.connect(
            lambda: self.controller.go_to_frame(self.sl.value())
        )
...
class GoTo(QWidget):
    ...
    def __init__(self, controller, nb_frames: int):
        ...
        gobut.setStyleSheet(
            "background-color : rgb(93,177,130); border-radius: 4px; min-height: 20px; min-width: 50px"
        )
...
class DashboardTab(QWidget):
    def __init__(self, controller, dashboard_chunk_size):
        ...
        self.chunknumber = 0
        self.current_i = (
            0  # the line number of the current time t, i.e. t%self.chunksize
        )
        self.grid = QGridLayout()
        ...
        current_btn.setStyleSheet(
            "background-color : rgb(42,99,246); border-radius: 4px;"
        )
        ...
        topscrollarea.setMinimumWidth(
            ...
            + topscrollarea.verticalScrollBar().sizeHint().width()
            + 10
        )
    ...
    def resizeEvent(self, event):
        ...
        button_width = min(
            50, max(30, self.width() // 10)
        )  # Scale width with widget, but keep within bounds
    ...
    def _make_button_press_function_t(self, i):
        def button_press_function():
            self.controller.go_to_frame(
                np.clip(self.chunksize * self.chunknumber + i, 0, self.T - 1)
            )
    ...
    def change_neuron_keys(self, key_changes):
        ...
        for idx_from1, key in key_changes:
            if (
                key is None and idx_from1 in self.button_columns
...
class TrackTab(QWidget):
    def __init__(self, controller):
        ...
        self.run_button.setStyleSheet(
            "background-color : rgb(93,177,130); border-radius: 4px; min-height: 20px"
        )
    ...
    def run(self, method_name, params):
        ...
        process = Process(
            target=tracking_methods.run,
            args=(method_name, command_pipe_sub, dataset_path, params),
        )
        process.start()
