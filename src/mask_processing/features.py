...
class FeatureBuilder:
    ...
    def extract_features(self, frames):
        ...
        for t in tqdm(
            frames
        ):  # Todo: parallelize? (probably needs batches to get rid of classes)
            if (
                self.data.use_seg_for_feature
            ):  # MB added to get features from both segmentations or mask matrix
                segmented = self.data.segmented_frame(t)
            else:
                segmented = self.data.get_mask(
                    t, force_original=True
                )  # Todo: ideally, should work with segs instead of neurons
            ...
            if self.image_data is not None:
                ...
                all_segs_data = get_all_segs_data(
                    all_segs_bin, dimensions
                ...
                all_segs_data = None
                rawimage_data = get_rawimage_data(
                    im_red, dimensions
                )  # MB changed it from None
            # Calculate features
            for s in np.unique(segmented)[
                1:
            ]:  # exclude  the first unique element which should always be 0
                ...
                if (
                    len(np.argwhere(segmented == s)) > 2
                ):  # MB added the if condition to avoid errors
                    ftr_dict = calculate_features(
                        segmented == s,
                        im_red,
                        dimensions,
                        all_segs_info=rawimage_data,
                    )
...
def compute_principal_axes(xyz_centered, weights=None, twodim=True):
    ...
    cov = np.cov(
        xyz_centered, aweights=weights
    )  # covariance between the variables x,y,z. pixels are the observations
    evals, evecs = np.linalg.eig(
        cov
    )  # MB: it seems to be for finding the main axis of the worm
...
def calculate_features(binary, im_red, dimensions, all_segs_info=None):
    ...
    if len(xyz) < 2:  # Todo: how can this happen??
        ...
        }  # Todo: what will happen to nans?
    m = im_red[xyz[:, 0], xyz[:, 1], xyz[:, 2]]

    centre_of_mass, wixx, wiyy, wizz, wixy, wixz, wiyz = calculate_moments(
        xyz, dimensions, m
    )

    if all_segs_info is not None:
        # TODO: use noseg or not
        total_center_of_mass = all_segs_info[
            "center_of_mass_noseg"
        ]  # TODO: make sure axes are computed using dimensions
        ...
        if ax3 is None:
            rot_mat = np.linalg.inv(
                np.vstack([ax1, ax2])
            )  # trransformation matrix to the coordinates where ax1 and ax2 are the x and y axis
        ...
        (
            ...
            rot_inv_xyz, np.array([1, 1, 1]), m
        )  # MB: why is the dimension set to [1,1,1]
    ...
    if all_segs_info is not None:
        feature_dict.update(
            {
                "Relative x location": x_loc,
                "Relative y location": y_loc,
                "Relative z location": z_loc,
                "Rot. Inv. x loc": rot_inv_x_loc,
                "Rot. Inv. y loc": rot_inv_y_loc,
                "Rot. Inv. z loc": rot_inv_z_loc,
            }
        )
        feature_dict.update(
            {
                "Rot. Inv. Weighted Ixx": rot_inv_wixx,
                "Rot. Inv. Weighted Iyy": rot_inv_wiyy,
                "Rot. Inv. Weighted Izz": rot_inv_wizz,
                "Rot. Inv. Weighted Ixy": rot_inv_wixy,
                "Rot. Inv. Weighted Ixz": rot_inv_wixz,
                "Rot. Inv. Weighted Iyz": rot_inv_wiyz,
            }
        )
...
def calculate_moments(xyz, dimensions, m, norm=1e9):
    ...
    xyz_dist = (
        xyz - centre_of_mass
    ) * dimensions  # why *dimensions?? it makes the wide dimension count more..??
