...
class Register_Rotate:
    ...
    def __init__(
        ...
        if not len(ref_frames):
            warnings.warn(
                "No reference provided and none already used; using first frame as reference."
            )
            ref_frames = [0]
        ...
        all_transformed = (
            self.data.get_transformation_keys()
        )  # the list of frames that have transformation matrix saved
        self.ref_frames_remaining = list(set(self.ref_frames) - set(all_transformed))
        self.frames_to_register = list(
            set(frames_to_register) - set(self.ref_frames)
        )  # MB: changed  and removed - set(all_transformed)  to register frames multiple times
        self.segments = dict()
        orig_ref = (
            self.data.base_ref_frame()
        )  # TODO MB: solve the issue with orig ref assignment
        if orig_ref is None:
            orig_ref = self.ref_frames[0]
        if orig_ref not in self.ref_frames:
            warnings.warn(
                "Cannot use same base reference frame as has been used before, which is {}.".format(
                    orig_ref
                )
    ...
    def transform_batch(self):
        ...
        if ParallelCompute:
            ...
            trans_to_refs = {
                refFr: self.data.get_transformation(refFr) for refFr in self.ref_frames
            }
            for batch in batches:
                non_ref = set(batch) - set(self.ref_frames)
                all_frames = list(
                    set(non_ref) - set(self.segments.keys())
                )  # MB: is this line necessary?
                ...
                sequence = zip(
                    images,
                    repeat(self.ref_frames),
                    repeat(ref_segs),
                    repeat(origRef),
                    repeat(trans_to_refs),
                    segs,
                )
    ...
    def transform_one(self, frame, ref_frames, segments=None):
        ...
        for ref_frame in ref_frames:
            ...
            points, ref_seg, after_tps, func_min = self.registration_JV2D(
                points, ref_seg, ctrl_pts
            )
            Initial = points
            final = after_tps
        ...
        self.data.save_ref(
            frame, min_frame
        )  # save the reference frames coresponding to the current frame
    ...
    def registration_JV(self, points, reference, ctrl_pts):
        ...
        [points, c_m, s_m] = core.normalize(
            points
        )  # translate and scale a point set so it has zero mean and unit variance
        [reference, c_s, s_s] = core.normalize(reference)
        [ctrl_pts, c_c, s_c] = core.normalize(ctrl_pts)
        after_tps, x0, loss = self.run_multi_level(
    ...
    def registration_JV2D(self, points, reference, ctrl_pts):
        ...
        iters = [
            ...
            500,
            300,
        ]  # the max number of function evaluations at each level, changed itteer from 500 to 100
        [points, c_m, s_m] = core.normalize(
            points
        )  # translate and scale a point set so it has zero mean and unit variance
        [reference, c_s, s_s] = core.normalize(reference)
    ...
    def run_multi_level(self, model, scene, ctrl_pts, level, scales, lambdas, iters):
        ...
        [basis, kernel] = core.prepare_TPS_basis(
            model, ctrl_pts
        )  # basis for performing TPS transformation
        loss = 1
    ...
    def add_good_to_ground_truth(self, n_display=6):
        ...
        times, dists = [
            list(tpl) for tpl in zip(*sorted(all_dists.items(), key=lambda x: x[1]))
        ]
    ...
    def load_single_contour(self, frame):
        ...
        if (
            len(np.unique(frame_segment)) < 3
        ):  # MB added to avoid error when facing empty frames
...
def transform_one(frame, ref_frames, ref_segs, origRef, trans_to_refs, segments=None):
    ...
    for r in range(len(ref_frames)):
        ...
        points, ref_seg, after_tps, func_min = registration_JV2D(
            points, ref_seg, ctrl_pts
        )
        Initial = points
        final = after_tps
    ...
    if min_frame != origRef:  # MB: is this necessary?
        ...
        ref_to_orig = trans_to_refs[
            min_frame
        ]  # self.data.get_transformation(min_frame)
