...
class Classification(AssignmentFinderInterface):
    ...
    def prepare(self, training_frames, *args, **kwargs):
        ...
        """
        training_segments, training_neurites = (
            self.annotations.get_segs_and_assignments(training_frames)
        )
        real_neurites = (
            self.annotations.real_neurites
        )  # MB: real neutit property is in h5data. it is a list from 1 to nb_neurons
        self.algo.train(
            training_segments,
            training_neurites,
            real_neurites,
...
class KNNClf(ClfAlgoInterface):
    ...
    def train(
        ...
        **kwargs
        ...
        self.logger.debug(
            ...
            + "with training_segments {} and real neurites {}".format(
                training_segments, neurites
            )
        )
        self.neurites = neurites
        self.classifiers = {}
        X_train, segMB = self.get_scaled_features(
            training_segments, segments=1
        )  # MB added seg_list to extract only the features
        print("X_train shape")
        print(X_train.shape)  # MB added
        ys = [
            np.array(training_neurites) == neu for neu in neurites
        ]  # MB: it works like a  set of functions(for each neu one function) on matrix of features(data matrix)
        # print(ys)
        w = "distance"
        algo = "ball_tree"
        ls = 20
        ...
        for neu in range(len(self.neurites)):  # for each neurite...
            best = 0
            for nn in np.logspace(
                np.log10(2), np.log10(25), 7
            ):  # ... small grid search on number of neighbours ...
                nn = int(nn)
                ...
                for fold in range(5):  # ... with 5-fold cross validation
                    knc = KNeighborsClassifier(
                        n_neighbors=nn, algorithm=algo, weights=w, leaf_size=ls
                    )
                    X_tr, X_te, y_tr, y_te = train_test_split(
                        X_train, ys[neu], test_size=0.2, random_state=fold
                    )  # divide the the data ((t,s) sets, actually theircorresponding features and neurit label)
                    knc.fit(X_tr, y_tr.ravel())  # MB changed y_tr to y_tr.ravel()
                    y_pre = knc.predict(X_te)
                    scores.append(
                        balanced_accuracy_score(y_te, y_pre)
                    )  # MB This line causes a warning: 'y_pred contains classes not in y_true'--This can be caused because y_te does not incluse some neurits
            ...
            knc = KNeighborsClassifier(
                n_neighbors=best_nn, algorithm=algo, weights=w, leaf_size=ls
            )
        ...
        log_fun(
            "Total time for hyperparameter optimization: {}".format(time.time() - start)
        )
        log_fun("Best nb_neighbours: {}".format(best_nns))
        log_fun("Accuracy on test set: {}".format(np.mean(best_accuracies)))
        best_sep_disp = {
            self.neurites[i]: best_accuracies[i] for i in range(len(self.neurites))
        }
    ...
    def classify(self, frames, verbose=True):
        ...
        if verbose:
            ...
            log_fun = self.logger.debug
        X_pred, segments = self.get_scaled_features(
            frames, times=1
        )  # MB add times in arguments
        y_pred_allneurites = [
            None for _ in segments
        ]  # MB:the array including the neurite assigned to each (t,s) with initial value of None
        for neu in self.neurites:
            ...
            for seg_idx in range(len(segments)):
                if y_pred[
                    seg_idx
                ]:  # if the classifiers output value for this neu is 1:
                    if (
                        y_pred_allneurites[seg_idx] is None
                    ):  # if the segment corresponding to seg_idx is not already assigned a neu
                        y_pred_allneurites[seg_idx] = neu
                    else:
                        log_fun(
                            "seg {} assigned to both {} and {}.".format(
                                segments[seg_idx],
                                y_pred_allneurites[seg_idx],
                                neu,
                            )
                        )
        ...
        log_fun(
            "Assigned: {} segments found".format(
                len(y_pred_allneurites) - nb_unassigned
            )
        )
    ...
    def get_scaled_features(
        self, frames, segments=None, times=None
    ):  # extra arguments was added by MB for classification purpose
        ...
        ):  # MB added, because the get_scaled is called by different functions with different format of frames argument
            features, segs_list = self.features.feature_array(
                segments=frames, segs_list=True
            )  # MB changed time to segments
        else:
            features, segs_list = self.features.feature_array(
                times=frames, segs_list=True
            )
