...
for i in range(T):
    ...
    dset = h5.create_dataset(
        str(i) + "/frame", (C, W, H, D), dtype="i2", compression="gzip"
    )
    dset[...] = (np.random.random((C, W, H, D)) * 255).astype(np.int16)  # int16 is i2
    dset = h5.create_dataset(
        str(i) + "/mask", (W, H, D), dtype="i2", compression="gzip"
    )
...
dset[...] = (
    np.random.random((T, N_neurons + 1, 3)) * np.array([W, H, D])[None, None, :]
).astype(np.float32)
dset[:, 0] = np.nan
...
ax.plot(
    mean_intensities[:210, 0] - np.mean(mean_intensities[:, 0]),
    label="Channel 0",
    color="blue",
    alpha=0.5,
)
ax.plot(
    mean_intensities[:210, 1] - np.mean(mean_intensities[:, 1]),
    label="Channel 1",
    color="red",
    alpha=0.5,
)
...
def find_small_valley_minima(
    ...
    minima, _ = find_peaks(
        -smoothed_data, distance=distance, prominence=prominence, width=width
    )
...
def plot_intensity_vs_frame(arr, start_idx=20, frames_per_volume=40, title=None):
    ...
    ax.plot(
        xx,
        mean_intensities[:, 0] - np.mean(mean_intensities[:, 0]),
        label="Channel 0",
        color="blue",
        alpha=0.5,
    )
    ax.plot(
        xx,
        mean_intensities[:, 1] - np.mean(mean_intensities[:, 1]),
        label="Channel 1",
        color="red",
        alpha=0.5,
    )
...
with tqdm(total=total_iterations, desc="Applying DoG filter") as pbar:
    for t in range(T):
        for c in range(C):
            for d in range(D):
                filtered_array[t, c, d] = dog_filter_2d(
                    aligned_array[t, c, d], sig1, sig2
                )
                pbar.update(1)
...
with h5py.File(os.path.join(output_path, filename + ".h5"), "w") as h5:
    ...
    for i in range(T):
        ...
        dset = h5.create_dataset(
            f"{i}/frame", (C, W, H, D), dtype="i2", compression="gzip"
        )
        dset[...] = np.transpose(
            filtered_array[i], (0, 2, 3, 1)
        )  # Reorder dimensions to (C, W, H, D)
