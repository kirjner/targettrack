...
class HarvardLab:
    def __init__(self, controller, dataset, settings):
        ...
        self.ker_sizexy = int(
            self.settings["calcium_intensity_kernel_xy"]
        )  # plus 1 for the errors
        ...
        if ci_int is None:
            self.ci_int = np.full(
                (self.controller.n_neurons, self.controller.frame_num, 2),
                np.nan,
                dtype=np.float32,
            )
            self.correct_existed = False
        elif (
            ci_int.shape[0] != self.controller.n_neurons
            or ci_int.shape[1] != self.controller.frame_num
        ):
            self.ci_int = np.full(
                (self.controller.n_neurons, self.controller.frame_num, 2),
                np.nan,
            ...
            self.correct_existed = False
            print(
                "WARNING: existing calcium intensities have incorrect shape; must recompute."
            )
        else:
            self.ci_int = ci_int[:, :, :2].astype(
                np.float32
            )  # some older saved values may have more than 2 in the last dimension
    ...
    def _update_single_ci_from_poindat(self, i, ind, loc, valid, im_r, im_g):
        ...
        int_rs = (
            np.sum(
                self.calcium_intensity_kernel_selectors
                * (im_r[loc_ker[0], loc_ker[1], loc_ker[2]][None, :]),
                axis=1,
            )
            / self.cikernelnorm
        )
        if self.channel_num == 2:
            int_gs = (
                np.sum(
                    self.calcium_intensity_kernel_selectors
                    * (im_g[loc_ker[0], loc_ker[1], loc_ker[2]][None, :]),
                    axis=1,
                )
                / self.cikernelnorm
            )
    ...
    def _update_ci_from_pointdat(self, dataset, t, i_from1):
        ...
        for i in iterator:
            ...
            if i_from1 is not None:
                self._update_single_ci_from_poindat(
                    i,
                    i_from1,
                    pointdat[i, i_from1],
                    valid=None,
                ...
                for j, (loc, valid) in enumerate(zip(locs, valids)):
                    self._update_single_ci_from_poindat(
                        i, j, loc, valid=valid, im_r=im_r, im_g=im_g
                    )
    ...
    def _update_ci_from_masks(self, dataset, t, i_from1):
        ...
        for i in iterator:
            ...
            if i_from1 is not None:
                self._update_single_ci_from_mask(
                    i,
                    i_from1,
                    i_from1 in mask,
                    mask=(mask == i_from1),
                ...
                for neu in self.controller.present_neurons_at_time(i):
                    self._update_single_ci_from_mask(
                        i, neu, True, mask=(mask == neu), im_g=im_g
                    )
    ...
    def change_nb_neurons(self, nb_neurons):
        ...
        if current_nb_neurons < nb_neurons:
            self.ci_int = np.full(
                (nb_neurons, self.controller.frame_num, 2),
                np.nan,
                dtype=np.float32,
            )
