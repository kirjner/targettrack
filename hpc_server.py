...
class H5StreamService(rpyc.Service):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                h5file = (
                    ...
                    if h5file is None or file_id_old != file_id
                    else h5file
                )
                if h5file is None:
    ...
    def exposed_create_dataset(
        ...
        try:
            ...
            h5file.create_dataset(
                path, shape=shape, dtype=dtype, compression=compression
            )
    ...
    def exposed_write_dataset(
        self, file_id: str, path: str, data: List[List[List[float]]]
    ):
        try:
            h5file = self.file_manager.get_file(file_id)
    ...
    def exposed_update_dataset_pointdat(
        self,
        file_id,
        patch_data,
        path="/pointdat",
        ...
        try:
            ...
            logger.info(
                f"Updated dataset at frame {frame}, neuron {neuron} with {coord}"
            )
        except Exception as e:
            logger.error(f"Error updating dataset: {str(e)}")

    def exposed_update_dataset_ci_int_t(
        self, file_id, patch_data, settings, path="/ci_int"
    ):
        ...
        try:
            ...
            if path not in h5file.keys():
                h5file.create_dataset(
                    path,
                    shape=(num_neurons, num_frames, 2),
                    dtype=np.float32,
                    compression=None,
                )
            ...
            calcium_analyzer = CalciumAnalyzer(
                frame_shape=img_shape,
                num_neurons=num_neurons,
                num_frames=num_frames,
                settings=settings,
            )
            total_changes = calcium_analyzer.update_ci_pointdata(
                pointdat=h5file["pointdat"], frame=image, t=frame
            )
    ...
    def exposed_get_helper_keys(
        self, file_id: str, prefix: str = "helper_"
    ) -> List[str]:
        """
    ...
    def exposed_get_dataset_chunk(
        self, file_id: str, path: str, slice_info: List[Tuple[int, int, int]]
    ) -> Optional[np.ndarray]:
        """
        ...
        try:
            ...
            if path.endswith("/frame"):
                ...
                for i in range(
                    max(0, int(path.split("/")[0]) - 20),
                    min(int(path.split("/")[0]) + 20, max_frames),
                ):
            ...
            logger.error(
                f"Error retrieving chunk from file {file_id}, path {path}, slices {slice_info}: {e}"
            )
            return None
