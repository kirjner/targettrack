...
def random_deformation(sh, device, scale=1, **kwargs):  # returns a zero deformation
    return (
        torch.randn(3, *sh, dtype=torch.float32, device=device) * scale,
        True,
    )
...
deformations = {
    "lowk2d": "get_deformation_lowk2d",  # this one is ours
...
class FourierLowK(nn.Module):
    ...
    def hermitian_symmetric(self, f_k_half):
        if self.n_d == 2:
            f_k_half[self.mids[0] + 1 :, 0] = torch.flip(
                f_k_half[1 : self.mids[0], 0], dims=(0,)
            ).conj()
            f_k_half[self.mids[0] + 1 :, self.mids[1]] = torch.flip(
                f_k_half[1 : self.mids[0], self.mids[1]], dims=(0,)
            ).conj()
        elif self.n_d == 3:
            f_k_half[self.mids[0] + 1 :, 0, 0] = (
                torch.flip(f_k_half[1 : self.mids[0], 0, 0], dims=(0,)).conj().clone()
            )
            f_k_half[self.mids[0] + 1 :, 0, self.mids[2]] = (
                torch.flip(f_k_half[1 : self.mids[0], 0, self.mids[2]], dims=(0,))
                .conj()
                .clone()
            )
            f_k_half[self.mids[0] + 1 :, self.mids[1], 0] = (
                torch.flip(f_k_half[1 : self.mids[0], self.mids[1], 0], dims=(0,))
                .conj()
                .clone()
            )
            f_k_half[self.mids[0] + 1 :, self.mids[1], self.mids[2]] = (
                torch.flip(
                    f_k_half[1 : self.mids[0], self.mids[1], self.mids[2]],
                    dims=(0,),
                )
                .conj()
                .clone()
            ...
            f_k_half[0, self.mids[1] + 1 :, 0] = (
                torch.flip(f_k_half[0, 1 : self.mids[1], 0], dims=(0,)).conj().clone()
            )
            f_k_half[0, self.mids[1] + 1 :, self.mids[2]] = (
                torch.flip(f_k_half[0, 1 : self.mids[1], self.mids[2]], dims=(0,))
                .conj()
                .clone()
            )
            f_k_half[self.mids[0], self.mids[1] + 1 :, 0] = (
                torch.flip(f_k_half[self.mids[0], 1 : self.mids[1], 0], dims=(0,))
                .conj()
                .clone()
            )
            f_k_half[self.mids[0], self.mids[1] + 1 :, self.mids[2]] = (
                torch.flip(
                    f_k_half[self.mids[0], 1 : self.mids[1], self.mids[2]],
                    dims=(0,),
                )
            ...
            f_k_half[self.mids[0] + 1 :, self.mids[1] + 1 :, 0] = (
                torch.flip(
                    f_k_half[1 : self.mids[0], 1 : self.mids[1], 0],
                    dims=(0, 1),
                )
                .conj()
                .clone()
            )
            f_k_half[self.mids[0] + 1 :, 1 : self.mids[1] :, 0] = (
                torch.flip(
                    f_k_half[1 : self.mids[0], self.mids[1] + 1 :, 0],
                    dims=(0, 1),
                )
                .conj()
                .clone()
            )
            f_k_half[self.mids[0] + 1 :, self.mids[1] + 1 :, self.mids[2]] = (
                torch.flip(
                    f_k_half[1 : self.mids[0], 1 : self.mids[1], self.mids[2]],
                    dims=(0, 1),
                )
                .conj()
                .clone()
            )
            f_k_half[self.mids[0] + 1 :, 1 : self.mids[1] :, self.mids[2]] = (
                torch.flip(
                    f_k_half[1 : self.mids[0], self.mids[1] + 1 :, self.mids[2]],
                    dims=(0, 1),
                )
                .conj()
                .clone()
    ...
    def init_feed_ind(self, k_cut_dimless):
        ks = [
            np.fft.fftfreq(d, 1 / d) for d in self.shape[:-1]
        ]  # descrete fourier transform sample frequencies
        ks.append(np.arange(self.shape[-1]))
        self.kgrid = np.array(
            np.meshgrid(*ks, indexing="ij")
        )  # grids of the fourier space
        self.ksqabs = np.sum(self.kgrid**2, axis=0)
        arrs = [torch.arange(d) for d in self.shape]
        unique_holders = torch.stack(
            torch.meshgrid(*arrs, indexing="ij")
        )  # grids of the image
        holders_hs = torch.stack(
            [
                self.hermitian_symmetric(unique_holder.clone())
                for unique_holder in unique_holders
            ]
        )
        ...
        self.dim = torch.sum(
            feed_ind
        ).item()  # converts the sum of all elements into python scaler
    ...
    def forward(self, x):
        ...
        x = x.reshape(b * c, -1)
        arr = torch.zeros(
            (x.size(0), 2, *self.shape), dtype=torch.float32, device=x.device
        )
        arrcomps = torch.zeros(
            (x.size(0), *self.shape), dtype=torch.complex64, device=x.device
        )
        arr[:, self.feed_ind] = x
        arrcomps.real += arr[:, 0]
        arrcomps.imag += arr[:, 1]
        ...
        return torch.fft.irfftn(
            self.f_k, s=self.realshape
        )  # computes n dimensional inverse descrete fourier transform of the real input- imaginary components ignored

    def to(self, **kwargs):
        self.feed_ind = self.feed_ind.to(**kwargs)


def get_deformation_lowk2d(
    ptfrom,
    ptto,
    sh,
    k_cut_dimless=2.5,
    ...
    locs_gridded = (
        2
        * (
            torch.tensor(locs)[:, None, None, :]
            / (np.array([W, H])[None, None, None, :] - 1)
        )
        - 1
    )  # MB: I think with None we add unit axis
    ...
    for iters in range(iterations):
        ...
        vecs_sampled = torch.nn.functional.grid_sample(
            deformation.repeat(locs_gridded.size(0), 1, 1, 1),
            locs_gridded,
            mode="bilinear",
            padding_mode="border",
            align_corners=True,
...
def deform(image, deformation, mask=None):
    C, W, H, D = image.shape
    grid = torch.stack(
        torch.meshgrid(*[torch.arange(s) for s in (W, H, D)], indexing="ij"),
        dim=0,
    )
...
def updim_deformation(deformation, D):
    deformation = torch.cat(
        [deformation, torch.zeros_like(deformation[[0]])], dim=0
    )  # 2,W,H and 1,W,H -> 3,W,H
